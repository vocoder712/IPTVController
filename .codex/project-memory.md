...
任务（2026-07-25）：将干预改为 30 秒周期，每周期随机阻断 1–26 秒，家长可
设置随机最小/最大值；剩余接通窗口检查载波，连续两个窗口无载波才认为电视
关闭。

状态：等待用户确认周期相位、两窗口规则适用范围和旧单值配置迁移方式。

已识别的实现影响：

- 需要把单一 `DownDuration` 改为最小/最大阻断秒数，并为随机源提供可注入
  接口，确保本地测试可完全确定。
- 30 秒周期的接通窗口最短仅 4 秒，现有 30 秒常规轮询不能可靠采样；状态机
  必须在窗口结束动作点主动刷新一次 DBus 状态，而不能使用可能来自自动 down
  阶段的旧缓存。
- 需要持久化最小/最大值，并定义当前设备旧 `block_seconds=15` 如何迁移。
- 需要明确 watching 尚未到限时的关机行为是否继续沿用上一版“立即进入
  30 分钟 cooldown”，还是也改为连续两个采样窗口。

用户确认：

- 每个 30 秒周期采用“先随机阻断，再接通剩余窗口”；自动达到限时和家长
  手动干预都立即开始首次随机阻断。
- “连续两个接通窗口无载波”只用于干预阶段；watching 尚未到限时时仍在
  检测到关机/拔线后立即进入 30 分钟 cooldown。
- 不兼容旧配置。本次以及以后结构性配置升级均直接清空旧持久配置，不做字段
  迁移。本次部署后恢复程序默认：智能限时关闭、观看上限 20 分钟、随机阻断
  范围 1–26 秒。

实现与验证进展（2026-07-25）：

- 核心状态机已改为 30 秒周期，先随机 down、后 up；随机范围默认 1–26 秒，
  使用可注入随机源，动作失败重试保留本周期已选时长。
- 干预接通窗口中途忽略载波，窗口结束由 runner 主动读取新鲜 DBus 状态；
  连续两个窗口无载波才进入 cooldown，任一窗口有载波即清零计数。
- 自动到限时和家长“立即干预”均立即开始首次 down；修改参数保留当前计时、
  干预相位、当前周期和 cooldown，新范围从下一周期生效。
- API/H5 改用必填的 `block_min_seconds`、`block_max_seconds`，不兼容旧
  `block_seconds`。
- 持久化版本升至 2；CRC 正确但版本不匹配的旧日志会被识别并删除。干预
  up/down 统一持久化为重启后重新开始 down，避免每 30 秒相位切换写闪存。
- 本地 `go test ./...`、`go test -race ./...`、`go vet ./...`、
  `git diff --check`、进程级 H5/API 模拟和 GOARM=5 交叉构建均通过。
- ARMv5 二进制 SHA-256：
  `cbef2e594edb6267f2ef03c2d10699dfa61036fa21f6128b38e3a2909285cba8`；
  `bootshell.sh` SHA-256：
  `50b426242cd711bed0e0de74015a08b62ca94b646002a0539a4beaad20fa27df`。
- 文件已通过 Telnet 流式上传、设备端双哈希和脚本语法校验后原子替换，
  旧 `state.log` 在首次重启前删除；新服务已由 `bootshell.sh + 重启光猫`
  启动，`/healthz` 返回 `real_control=true`，新 API 字段和
  `dbus_lan2_status` 已确认。
- 首次重启后 API 返回“启用、28 分钟、18–26 秒”；用户确认这是其手动设置，
  不是旧配置回流，因此保留该配置且不覆盖为程序默认值。
- 最终只读验收：`/healthz` 为健康且 `real_control=true`；H5 包含最小/最大
  随机阻断控件；状态为 `idle`、`carrier=0`，来源为
  `dbus_lan2_status`；用户设置完整保留。
- 在再次核对正式二进制和 `bootshell.sh` 哈希后，已删除本次
  `.iptv-control.rollback` 与 `.bootshell.sh.rollback` 临时回滚文件；持久
  分区剩余 21,344 KiB。

状态：已完成本地实现、真实设备部署和重启验收。
